...
with st.sidebar:
   ...
   st.image("YouTube-logo-with-shadow-PNG.png",use_column_width=True)
...
def Video_details_to_sql(channel_id):
    ...
    try:
         ...
         if 'Duplicate entry' in str(e):
            ...
            conn.rollback()
            #tab2.error(e)
            delete_query=f"DELETE FROM channel_details WHERE Channel_Id = '{channel_id}'"
...
def Comment_details_to_sql(channel_id):
    ...
    try:
         ...
         if 'Duplicate entry' in str(e):
            ...
            conn.rollback()
            my_bar.empty()
            tab2.warning("Sorry , Data Upload Unsuccessful . Try another Channel id")
...
if questions == '1. What are the names of all the videos and their corresponding channels?':
   ...
   with col2:
      fig1 = px.bar(df2, y='Video Count', x='Channel Name', text_auto='.2s', title='Channel with most number of Videos',color='Video Count',color_continuous_scale='teal',hover_data={'Video Count'} )
   ...
   with col1:
      ...
      tab3.dataframe(df3)
      # creating a pie chart and bar chart for the output
   with col2:
      fig2 = px.bar(df3, x='View Count', y='Video Name', title='Top 10 most viewed videos',color='View Count',color_continuous_scale='teal',text_auto='.2s',orientation='h')
      ...
      fig2 = px.pie(df3, values='View Count', names='Video Name', title='Top 10 most viewed videos',hole=0.4, color_discrete_sequence = px.colors.sequential.Teal)
      fig2.update_traces(textfont_size=16,marker=dict(line=dict(width=1, color='DarkSlateGrey')))
      fig2.update_layout(title_font_color='#616569',title_font=dict(size=25),coloraxis_showscale=True)
      tab3.plotly_chart(fig2,use_container_width=True)
   ...
   with col2:
    ...
    # creating a bar chart for the output
      fig_4 = px.bar(df_4, x='Comment Count', y='Video Name', title='Top 10 most Commented Videos',color='Comment Count',color_continuous_scale='teal',text_auto='.2s',orientation='h')
      fig_4.update_traces(textfont_size=16,marker=dict(line=dict(width=1, color='DarkSlateGrey')))
    ...
    # creating a pie chart for the output
      fig_4 = px.pie(df_4, values='Comment Count', names='Video Name', title='Top 10 most Commented Videos',hole=0.4, color_discrete_sequence = px.colors.sequential.Teal)
   ...
   with col2:
      ...
      # creating a bar chart for the output
      fig_4 = px.bar(df_5, x='No of Likes', y='Name of the Video', title='Top 10 most Liked Videos',color='No of Likes',color_continuous_scale='teal',text_auto='.2s',orientation='h')
      fig_4.update_traces(textfont_size=16,marker=dict(line=dict(width=1, color='DarkSlateGrey')))
      fig_4.update_layout(title_font_color='#616569',title_font=dict(size=25),coloraxis_showscale=True)
      tab3.plotly_chart(fig_4,use_container_width=True)
      # creating a pie chart for the output
      fig_4 = px.pie(df_5, values='No of Likes', names='Name of the Video', title='Top 10 most Liked Videos',hole=0.4, color_discrete_sequence = px.colors.sequential.Teal )
      fig_4.update_traces(textfont_size=14,marker=dict(line=dict(width=1, color='DarkSlateGrey')))
      fig_4.update_layout(title_font_color='#616569',title_font=dict(size=25),coloraxis_showscale=True)
      tab3.plotly_chart(fig_4,use_container_width=True)
   ...
   with col1:
      ...
      df9['Duration_seconds'] = df9['Average Duration of Videos'].apply(lambda x: int(pd.to_timedelta(x).total_seconds()))
      #df9['Average Duration of Videos'] = pd.to_datetime(df9['Average Duration of Videos'],format = "%H:%M:%S")
      df9.index += 1
      tab3.dataframe(df9)

   with col2:
      # creating a bar chart for the output
      fig4 = px.bar(df9, y='Channel Name',x='Average Duration of Videos', title='Average Duration of Videos', color='Duration_seconds',color_continuous_scale='teal',hover_data= {'Channel Name':True,'Average Duration of Videos': True})
   ...
   with col2:
      ...
      # creating a bar chart for the output
      fig_4 = px.bar(df_10, x='No of Comments', y='Name of the Video', title="Top 20 Videos with highest number of comments",color='No of Comments',color_continuous_scale='teal',text_auto='.2s',orientation='h')
      fig_4.update_traces(textfont_size=16,marker=dict(line=dict(width=1, color='DarkSlateGrey')))
      fig_4.update_layout(title_font_color='#616569',title_font=dict(size=25),coloraxis_showscale=True,height = 700)
      tab3.plotly_chart(fig_4,use_container_width=True)
